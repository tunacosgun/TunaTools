//! Takes comments from tuna_js_parser and turns them into test data.
...
pub fn generate_parser_tests(mode: Mode) -> Result<()> {
    let tests = tests_from_dir(&project_root().join(Path::new("crates/tuna_js_parser/src")))?;
    ...
    some_file_was_updated |=
        install_tests(&tests.ok, "crates/tuna_js_parser/test_data/inline/ok", mode)?;
    some_file_was_updated |= install_tests(
        &tests.err,
        "crates/tuna_js_parser/test_data/inline/err",
        mode,
    )?;

    if some_file_was_updated {
        let _ = filetime::set_file_mtime(
            "crates/tuna_js_parser/src/tests.rs",
