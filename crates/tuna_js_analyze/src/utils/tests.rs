...
use tuna_js_syntax::JsSyntaxNode;
use tuna_js_syntax::{
    AnyJsObjectMember, JsFileSource, JsFormalParameter, JsIdentifierBinding, JsLanguage,
    JsVariableDeclarator,
};
use tuna_rowan::{AstNode, BatchMutationExt, SyntaxNodeCast};
...
pub fn assert_rename_binding_a_to_b_ok(before: &str, expected: &str) {
    ...
    assert!(!tuna_js_parser::test_utils::has_bogus_nodes_or_empty_slots(
        &root
    ));
}
...
pub fn ok_find_attributes_by_name() {
    ...
    let list = r
        .syntax()
        .descendants()
        .find_map(tuna_js_syntax::JsxAttributeList::cast)
        .unwrap();
