...
use tuna_analyze::{merge_node_visitors, Visitor, VisitorContext};
use tuna_js_syntax::{
...
use tuna_rowan::{declare_node_union, AstNode, SyntaxError, SyntaxResult};
    ...
    ( $vis:vis $name:ident { $( $id:ident: $visitor:ty, )* } ) => {
        ...
        $( impl<'a> MergedVisitor<'a, $visitor> for StatementStack<'a> {
            fn read_top(self) -> SyntaxResult<&'a mut $visitor> {
                #[cfg(debug_assertions)]
                let (_, visitor) =
                    self.$id.1.last_mut().ok_or(::tuna_rowan::SyntaxError::MissingRequiredChild)?;
                #[cfg(not(debug_assertions))]
                let (_, visitor) =
                    self.$id.last_mut().ok_or(::tuna_rowan::SyntaxError::MissingRequiredChild)?;
...
impl tuna_analyze::NodeVisitor<ControlFlowVisitor> for FunctionVisitor {
...
/// Wrapper trait for [tuna_analyze::NodeVisitor] adding control flow specific
...
/// Wrapper type implementing [tuna_analyze::NodeVisitor] for types
/// implementing the control-flow specific [NodeVisitor] trait
pub(super) struct VisitorAdapter<V>(SyntaxResult<V>);

impl<V> tuna_analyze::NodeVisitor<ControlFlowVisitor> for VisitorAdapter<V>
where
    V: NodeVisitor,
{
    type Node = V::Node;
