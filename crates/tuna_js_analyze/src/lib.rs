...
use tuna_aria::{AriaProperties, AriaRoles};
use tuna_diagnostics::{category, Diagnostic, Error as DiagnosticError};
use tuna_js_syntax::suppression::SuppressionDiagnostic;
use tuna_js_syntax::{suppression::parse_suppression_comment, JsFileSource, JsLanguage};
...
#[cfg(test)]
mod tests {
    use tuna_analyze::options::RuleOptions;
    use tuna_analyze::{AnalyzerOptions, Never, RuleCategories, RuleFilter, RuleKey};
    use tuna_console::fmt::{Formatter, Termcolor};
    use tuna_console::{markup, Markup};
    use tuna_diagnostics::category;
    use tuna_diagnostics::termcolor::NoColor;
    use tuna_diagnostics::{Diagnostic, DiagnosticExt, PrintDiagnostic, Severity};
    use tuna_js_parser::{parse, JsParserOptions};
    use tuna_js_syntax::{JsFileSource, TextRange, TextSize};
    use std::slice;
...
impl tuna_console::fmt::Display for RuleError {
    fn fmt(&self, fmt: &mut tuna_console::fmt::Formatter) -> std::io::Result<()> {
        match self {
            RuleError::ReplacedRootWithNonRootError {
                rule_name: Some((group, rule)),
            } => {
