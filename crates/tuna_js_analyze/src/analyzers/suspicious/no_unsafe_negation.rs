...
use tuna_js_syntax::{AnyJsExpression, JsInExpression, JsInstanceofExpression, T};
use tuna_rowan::{declare_node_union, AstNode, AstNodeExt, BatchMutationExt};
    ...
    fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
        ...
        match node {
            JsInOrInstanceOfExpression::JsInstanceofExpression(expr) => {
                ...
                if let Some(unary) = left.as_js_unary_expression() {
                    match unary.operator().ok()? {
                        tuna_js_syntax::JsUnaryOperator::LogicalNot => Some(()),
                        _ => None,
                    }
                } else {
                    None
            ...
            JsInOrInstanceOfExpression::JsInExpression(expr) => {
                ...
                if let Some(tuna_js_syntax::AnyJsExpression::JsUnaryExpression(unary)) =
        ...
        match node {
            JsInOrInstanceOfExpression::JsInstanceofExpression(expr) => {
                ...
                let next_parenthesis_expression = make::js_parenthesized_expression(
                    make::token(T!['(']),
                    tuna_js_syntax::AnyJsExpression::JsInstanceofExpression(next_expr),
                    make::token(T![')']),
                );
            ...
            JsInOrInstanceOfExpression::JsInExpression(expr) => {
                ...
                let next_expr = expr.clone().replace_node_discard_trivia(
                    left.clone(),
                    tuna_js_syntax::AnyJsInProperty::AnyJsExpression(argument),
                )?;
                let next_parenthesis_expression = make::js_parenthesized_expression(
                    make::token(T!['(']),
                    tuna_js_syntax::AnyJsExpression::JsInExpression(next_expr),
                    make::token(T![')']),
                );
                let next_unary_expression = make::js_unary_expression(
                    unary_expression.operator_token().ok()?,
