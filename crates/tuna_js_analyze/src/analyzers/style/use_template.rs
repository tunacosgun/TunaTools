use tuna_analyze::RuleSuppressions;
use tuna_analyze::{context::RuleContext, declare_rule, ActionCategory, Ast, Rule, RuleDiagnostic};
use tuna_console::markup;
use tuna_diagnostics::Applicability;
use tuna_js_factory::make;
use tuna_js_syntax::AnyJsTemplateElement::{self, JsTemplateElement};
use tuna_js_syntax::{
...
use tuna_rowan::{AstNode, AstNodeList, BatchMutationExt, SyntaxToken};
            ...
            .any(|expr| {
                !matches!(
                    ...
                    AnyJsExpression::AnyJsLiteralExpression(
                        tuna_js_syntax::AnyJsLiteralExpression::JsStringLiteralExpression(_)
                    )
                )
            })
...
fn is_unnecessary_string_concat_expression(node: &JsBinaryExpression) -> Option<bool> {
    ...
    match node.left().ok()? {
        tuna_js_syntax::AnyJsExpression::JsBinaryExpression(binary) => {
            if is_unnecessary_string_concat_expression(&binary) == Some(true) {
                return Some(true);
            }
        }
        tuna_js_syntax::AnyJsExpression::JsTemplateExpression(_) => return Some(true),
        tuna_js_syntax::AnyJsExpression::AnyJsLiteralExpression(
            tuna_js_syntax::AnyJsLiteralExpression::JsStringLiteralExpression(string_literal),
        ) => {
...
fn has_new_line_or_tick(
    string_literal: tuna_js_syntax::JsStringLiteralExpression,
) -> Option<usize> {
