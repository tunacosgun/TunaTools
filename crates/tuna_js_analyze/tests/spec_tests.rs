use tuna_analyze::{AnalysisFilter, AnalyzerAction, ControlFlow, Never, RuleFilter};
use tuna_diagnostics::advice::CodeSuggestionAdvice;
use tuna_diagnostics::{DiagnosticExt, Severity};
...
fn run_test(input: &'static str, _: &str, _: &str, _: &str) {
    ...
    if tuna_js_analyze::metadata().find_rule(group, rule).is_none() {
...
pub(crate) fn analyze_and_snap(
    ...
    let (_, errors) = tuna_js_analyze::analyze(&root, filter, &options, source_type, |event| {
