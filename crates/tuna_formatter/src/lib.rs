...
/// Formatting trait for types that can create a formatted representation. The `tuna_formatter` equivalent
/// to [std::fmt::Display].
///
/// ## Example
...
///
/// ```
/// use tuna_formatter::{format, write, IndentStyle, LineWidth};
...
///
/// For example, the `tuna_js_formatter` crate isn't able to implement [Format] on `JsIfStatement`
/// because both the [Format] trait and `JsIfStatement` are external types (Rust's orphan rule).
///
/// That's why the `tuna_js_formatter` crate must define a new-type that implements the formatting
/// of `JsIfStatement`.
...
/// ## Examples
///
/// This can be useful if you want to format a `SyntaxNode` inside tuna_formatter.. `SyntaxNode` doesn't implement [Format]
...
/// ```ignore
/// use tuna_formatter::prelude::*;
/// use tuna_formatter::{format, Formatted, FormatWithRule};
/// use tuna_rowan::{Language, SyntaxNode};
...
/// ```
/// use tuna_formatter::prelude::*;
/// use tuna_formatter::{VecBuffer, format_args, FormatState, write, Formatted};
///
/// # fn main() -> FormatResult<()> {
