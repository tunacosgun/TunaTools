...
use tuna_diagnostics::{category, Category, Diagnostic, DiagnosticTags, Location, Severity};
use tuna_rowan::{SyntaxError, TextRange};
use std::error::Error;
...
impl Diagnostic for FormatError {
    ...
    fn message(
        &self,
        fmt: &mut tuna_diagnostics::console::fmt::Formatter<'_>,
    ) -> std::io::Result<()> {
        match self {
            FormatError::SyntaxError => fmt.write_str("Syntax error."),
            FormatError::RangeError { input, tree } => std::write!(
