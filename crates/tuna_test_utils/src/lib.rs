...
use tuna_analyze::{AnalyzerAction, AnalyzerOptions};
use tuna_console::fmt::{Formatter, Termcolor};
use tuna_console::markup;
use tuna_diagnostics::termcolor::Buffer;
use tuna_diagnostics::{DiagnosticExt, Error, PrintDiagnostic};
use tuna_json_parser::{JsonParserOptions, ParseDiagnostic};
use tuna_rowan::{SyntaxKind, SyntaxNode, SyntaxSlot};
use tuna_service::configuration::to_analyzer_configuration;
use tuna_service::settings::{Language, WorkspaceSettings};
...
pub fn create_analyzer_options(
    ...
    if let Ok(json) = std::fs::read_to_string(options_file.clone()) {
        let deserialized = tuna_deserialize::json::deserialize_from_json_str::<Configuration>(
            json.as_str(),
...
pub fn diagnostic_to_string(name: &str, source: &str, diag: Error) -> String {
    ...
    let text = markup_to_string(tuna_console::markup! {
...
fn markup_to_string(markup: tuna_console::Markup) -> String {
    let mut buffer = Vec::new();
    let mut write =
        tuna_console::fmt::Termcolor(tuna_diagnostics::termcolor::NoColor::new(&mut buffer));
...
extern "C" fn check_leaks() {
    if let Some(report) = tuna_rowan::check_live() {
        panic!("\n{report}")
    }
}
