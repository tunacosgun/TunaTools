...
use tuna_console::{markup, ConsoleExt};
use tuna_diagnostics::{category, PrintDiagnostic};
use tuna_fs::OpenOptions;
use tuna_json_parser::JsonParserOptions;
use tuna_json_syntax::JsonRoot;
use tuna_migrate::{migrate_configuration, ControlFlow};
use tuna_rowan::AstNode;
use tuna_service::workspace::FixAction;
use std::borrow::Cow;
...
pub(crate) fn run(
    ...
    let parsed = tuna_json_parser::parse_json(&configuration_content, JsonParserOptions::default());
    let mut errors = 0;
