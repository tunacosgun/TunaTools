...
use tuna_diagnostics::{category, Category, MAXIMUM_DISPLAYABLE_DIAGNOSTICS};
use tuna_fs::RomePath;
use tuna_service::workspace::{FeatureName, FixFileMode};
...
pub(crate) fn execute_mode(
    ...
    if let Some((path, content)) = mode.as_stdin_file() {
        ...
        std_in::run(session, &mode, tuna_path, content.as_str())
