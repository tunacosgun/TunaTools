...
use tuna_console::{fmt, markup, Console, ConsoleExt};
use tuna_diagnostics::{
    adapters::StdError, category, DiagnosticExt, Error, PrintDescription, PrintDiagnostic,
    Resource, Severity,
};
use tuna_fs::{FileSystem, PathInterner, RomePath};
use tuna_fs::{TraversalContext, TraversalScope};
use tuna_service::workspace::{FeaturesBuilder, IsPathIgnoredParams};
use tuna_service::{
...
impl<'ctx, 'app> TraversalOptions<'ctx, 'app> {
    ...
    pub(crate) fn miss_handler_err(&self, err: WorkspaceError, tuna_path: &RomePath) {
        self.push_diagnostic(
            StdError::from(err)
                .with_category(category!("files/missingHandler"))
                .with_file_path(tuna_path.display().to_string()),
        );
    }
}
...
impl<'ctx, 'app> TraversalContext for TraversalOptions<'ctx, 'app> {
    ...
    fn can_handle(&self, tuna_path: &RomePath) -> bool {
        if tuna_path.is_dir() {
            let can_handle = !self
                .workspace
        ...
        let file_features = match file_features {
            ...
            Err(err) => {
                self.miss_handler_err(err, tuna_path);
