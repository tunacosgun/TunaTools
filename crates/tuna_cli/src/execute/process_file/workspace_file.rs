...
use tuna_diagnostics::{category, Error};
use tuna_fs::{File, OpenOptions, RomePath};
use tuna_service::file_handlers::Language;
use tuna_service::workspace::{FileGuard, OpenFileParams};
use tuna_service::{Workspace, WorkspaceError};
...
impl<'ctx, 'app> WorkspaceFile<'ctx, 'app> {
    ...
    pub(crate) fn new(
        ...
        let tuna_path = RomePath::new(path);
