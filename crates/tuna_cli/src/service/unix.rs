...
use tuna_lsp::{ServerConnection, ServerFactory};
use tokio::{
    io::Interest,
    net::{
...
fn get_socket_name() -> PathBuf {
    env::temp_dir().join(format!("rome-socket-{}", tuna_service::VERSION))
}
...
pub(crate) fn enumerate_pipes() -> io::Result<impl Iterator<Item = String>> {
    fs::read_dir(env::temp_dir()).map(|iter| {
        iter.filter_map(|entry| {
            ...
            let tuna_version = file_name.strip_prefix("rome-socket")?;
            if tuna_version.is_empty() {
                Some(String::new())
            } else {
                Some(tuna_version.strip_prefix('-')?.to_string())
            }
        })
    })
}
