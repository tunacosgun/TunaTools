...
use tuna_cli::{
    open_transport, tuna_command, setup_panic_handler, to_color_mode, CliDiagnostic, CliSession,
    RomeCommand,
};
use tuna_console::{markup, ConsoleExt, EnvConsole};
use tuna_diagnostics::{set_bottom_frame, PrintDiagnostic};
...
fn main() -> ExitCode {
    ...
    let command = tuna_command().run_inner(Args::current_args());
    match command {
        Ok(command) => {
