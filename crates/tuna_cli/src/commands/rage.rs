use tuna_console::fmt::{Display, Formatter};
use tuna_console::{fmt, markup, ConsoleExt, HorizontalLine, Markup};
use tuna_diagnostics::termcolor::{ColorChoice, WriteColor};
use tuna_diagnostics::{termcolor, PrintDescription};
use tuna_fs::FileSystem;
use tuna_service::workspace::{client, RageEntry, RageParams};
use tuna_service::{load_config, ConfigurationBasePath, DynRef, Workspace};
...
impl Display for RunningRomeServer {
    fn fmt(&self, f: &mut Formatter) -> io::Result<()> {
        ...
        for version in versions {
            if version == tuna_service::VERSION {
