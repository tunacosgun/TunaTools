...
use tuna_service::configuration::organize_imports::OrganizeImports;
use tuna_service::configuration::{FormatterConfiguration, LinterConfiguration};
use tuna_service::workspace::UpdateSettingsParams;
use tuna_service::{Configuration, MergeWith};
...
pub(crate) struct CiCommandPayload {
    ...
    pub(crate) tuna_configuration: Configuration,
...
pub(crate) fn ci(mut session: CliSession, payload: CiCommandPayload) -> Result<(), CliDiagnostic> {
    ...
    configuration.merge_with(payload.tuna_configuration.files);
    configuration.merge_with(payload.tuna_configuration.vcs);
    configuration.merge_with_if(
        payload.tuna_configuration.formatter,
        !configuration.is_formatter_disabled(),
    );
    configuration.merge_with_if(
        payload.tuna_configuration.organize_imports,
        !configuration.is_organize_imports_disabled(),
    );
