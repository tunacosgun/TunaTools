...
use tuna_cli::{tuna_command, CliDiagnostic, CliSession};
use tuna_console::{markup, BufferConsole, Console, ConsoleExt};
use tuna_fs::{FileSystem, MemoryFileSystem};
use tuna_service::{App, DynRef};
...
pub(crate) fn run_cli<'app>(
    ...
    use tuna_cli::SocketTransport;
    use tuna_lsp::ServerFactory;
    use tuna_service::{workspace, WorkspaceRef};
    use tokio::{
        io::{duplex, split},
        runtime::Runtime,
    };
    ...
    let command = tuna_command().run_inner(args);
    match command {
