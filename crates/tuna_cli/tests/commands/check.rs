...
use tuna_console::{markup, BufferConsole, LogLevel, MarkupBuf};
use tuna_fs::{ErrorEntry, FileSystemExt, MemoryFileSystem, OsFileSystem};
...
fn shows_organize_imports_diff_on_check() {
    ...
    let tuna_json = r#"{ "organizeImports": { "enabled": true } }"#;

    let config_path = Path::new("rome.json");
    fs.insert(config_path.into(), tuna_json.as_bytes());
...
fn dont_applies_organize_imports_for_ignored_file() {
    ...
    let tuna_json = r#"{ "organizeImports": { "enabled": true, "ignore": ["check.js"] } }"#;
