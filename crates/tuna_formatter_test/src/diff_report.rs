...
impl DiffReport {
            ...
            Ok(value) if value == "1" => {
                if !Self::is_ignored(file_name) {
                    self.state.lock().unwrap().push(DiffReportItem {
                        file_name,
                        tuna_formatted_result: tuna_formatted_result.to_owned(),
    ...
    fn report_prettier(&self, report_type: ReportType, report_filename: String) {
        ...
        for DiffReportItem {
            file_name,
            tuna_formatted_result,
            prettier_formatted_result,
        } in state.iter()
        {
            file_count += 1;

            let tuna_lines = tuna_formatted_result.lines().count();
            let prettier_lines = prettier_formatted_result.lines().count();

            let (matched_lines, ratio, diff) = if tuna_formatted_result == prettier_formatted_result
            {
                (tuna_lines, 1f64, None)
            } else {
                ...
                let ratio = matched_lines as f64 / tuna_lines.max(prettier_lines) as f64;

                (matched_lines, ratio, Some(diff))
            };

            total_lines += tuna_lines.max(prettier_lines);
