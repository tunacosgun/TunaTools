...
use tuna_rowan::{AstNode, Language, SyntaxNode, TextRange, WalkEvent};
...
macro_rules! merge_node_visitors {
    ( $vis:vis $name:ident { $( $id:ident: $visitor:ty, )+ } ) => {
        ...
        impl $crate::Visitor for $name {
            type Language = <( $( <$visitor as $crate::NodeVisitor<$name>>::Node, )* ) as ::tuna_rowan::macros::UnionLanguage>::Language;

            fn visit(
                &mut self,
                event: &::tuna_rowan::WalkEvent<::tuna_rowan::SyntaxNode<Self::Language>>,
                mut ctx: $crate::VisitorContext<Self::Language>,
            ) {
                match event {
                    ::tuna_rowan::WalkEvent::Enter(node) => {
                        let kind = node.kind();

                        $(
                            if <<$visitor as $crate::NodeVisitor<$name>>::Node as ::tuna_rowan::AstNode>::can_cast(kind) {
                                let node = <<$visitor as $crate::NodeVisitor<$name>>::Node as ::tuna_rowan::AstNode>::unwrap_cast(node.clone());
                    ...
                    ::tuna_rowan::WalkEvent::Leave(node) => {
                        let kind = node.kind();
