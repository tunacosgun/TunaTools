...
use tuna_analyze::RuleCategories;
use tuna_console::markup;
use tuna_fs::{FileSystem, OsFileSystem, RomePath};
use tuna_service::workspace::{
    FeatureName, FeaturesBuilder, PullDiagnosticsParams, SupportsFeatureParams,
};
use tuna_service::workspace::{RageEntry, RageParams, RageResult, UpdateSettingsParams};
use tuna_service::{load_config, ConfigurationBasePath, Workspace};
use tuna_service::{DynRef, WorkspaceError};
...
impl Session {
    ...
    pub(crate) async fn update_diagnostics(&self, url: lsp_types::Url) -> Result<()> {
        let tuna_path = self.file_path(&url)?;
