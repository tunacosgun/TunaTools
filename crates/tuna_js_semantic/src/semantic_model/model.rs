use super::*;
use tuna_js_syntax::{AnyJsFunction, AnyJsRoot, JsInitializerClause, JsVariableDeclarator};
...
impl SemanticModel {
    ...
    /// use tuna_rowan::{AstNode, SyntaxNodeCast};
    /// use tuna_js_syntax::{JsFileSource, JsReferenceIdentifier};
    /// use tuna_js_semantic::{semantic_model, SemanticModelOptions, SemanticScopeExtensions};
    ///
    /// let r = tuna_js_parser::parse("function f(){let a = arguments[0]; let b = a + 1;}", JsFileSource::js_module(), JsParserOptions::default());
    ...
    /// use tuna_rowan::{AstNode, SyntaxNodeCast};
    /// use tuna_js_syntax::{JsFileSource, JsReferenceIdentifier};
    /// use tuna_js_semantic::{semantic_model, BindingExtensions, SemanticModelOptions};
    ///
    /// let r = tuna_js_parser::parse("function f(){let a = arguments[0]; let b = a + 1;}", JsFileSource::js_module(), JsParserOptions::default());
    /// let model = semantic_model(&r.tree(), SemanticModelOptions::default());
    ///
    ...
    /// use tuna_js_parser::JsParserOptions;
    /// use tuna_rowan::{AstNode, SyntaxNodeCast};
    /// use tuna_js_syntax::{JsFileSource, AnyJsFunction};
    /// use tuna_js_semantic::{semantic_model, CallsExtensions, SemanticModelOptions};
    ///
    /// let r = tuna_js_parser::parse("function f(){} f() f()", JsFileSource::js_module(), JsParserOptions::default());
    /// let model = semantic_model(&r.tree(), SemanticModelOptions::default());
    ///
    /// let f_declaration = r
    ///     .syntax()
