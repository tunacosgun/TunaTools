...
use tuna_console::{markup, ConsoleExt, EnvConsole};
...
use tuna_js_syntax::{AnyJsRoot, JsFileSource, JsSyntaxToken, TextRange, TextSize, WalkEvent};
use tuna_rowan::{AstNode, NodeOrToken};
...
pub fn assert(code: &str, test_name: &str) {
    let r = tuna_js_parser::parse(code, JsFileSource::tsx(), JsParserOptions::default());
...
impl SemanticAssertions {
    fn from_root(root: AnyJsRoot, code: &str, test_name: &str) -> Self {
        ...
        for node in root
            .syntax()
            .preorder_with_tokens(tuna_rowan::Direction::Next)
        {
