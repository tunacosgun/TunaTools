...
use tuna_analyze::{AnalyzerOptions, ControlFlow, Never, RuleCategories};
use tuna_deserialize::json::deserialize_from_json_ast;
use tuna_diagnostics::{category, Diagnostic, DiagnosticExt, Severity};
use tuna_formatter::{FormatError, Printed};
use tuna_fs::{RomePath, CONFIG_NAME};
use tuna_json_analyze::analyze;
use tuna_json_formatter::context::JsonFormatOptions;
use tuna_json_formatter::format_node;
use tuna_json_parser::JsonParserOptions;
use tuna_json_syntax::{JsonFileSource, JsonLanguage, JsonRoot, JsonSyntaxNode};
use tuna_parser::AnyParse;
use tuna_rowan::{AstNode, FileSource, NodeCache};
use tuna_rowan::{TextRange, TextSize, TokenAtOffset};
...
fn parse(
    ...
    let source_type =
        JsonFileSource::try_from(tuna_path.as_path()).unwrap_or_else(|_| match language_hint {
    ...
    let options: JsonParserOptions = JsonParserOptions {
        allow_comments: parser.allow_comments
            ...
            || is_file_allowed_as_jsonc(tuna_path),
    };
    let parse = tuna_json_parser::parse_json_with_cache(text, cache, options);
...
fn debug_formatter_ir(
    ...
    let options = settings.format_options::<JsonLanguage>(tuna_path);
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = tuna_json_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
fn format_on_type(
    ...
    let printed = tuna_json_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}
