...
use tuna_diagnostics::{category, Applicability, Diagnostic, DiagnosticExt, Severity};
use tuna_formatter::{FormatError, Printed};
use tuna_fs::RomePath;
use tuna_js_analyze::utils::rename::{RenameError, RenameSymbolExtensions};
use tuna_js_analyze::{
    analyze, analyze_with_inspect_matcher, visit_registry, ControlFlowGraph, RuleError,
};
use tuna_js_formatter::context::{
...
use tuna_js_semantic::{semantic_model, SemanticModelOptions};
use tuna_js_syntax::{
    AnyJsRoot, JsFileSource, JsLanguage, JsSyntaxNode, TextRange, TextSize, TokenAtOffset,
};
use tuna_parser::AnyParse;
use tuna_rowan::{AstNode, BatchMutationExt, Direction, FileSource, NodeCache};
use std::borrow::Cow;
use std::ffi::OsStr;
use std::fmt::Debug;
...
fn parse(
    ...
    let source_type =
        JsFileSource::try_from(tuna_path.as_path()).unwrap_or_else(|_| match language_hint {
    ...
    let parse = tuna_js_parser::parse_js_with_cache(text, source_type, options, cache);
...
fn debug_formatter_ir(
    ...
    let options = settings.format_options::<JsLanguage>(tuna_path);
...
fn lint(params: LintParams) -> LintResults {
        ...
        |signal| {
            if let Some(mut diagnostic) = signal.diagnostic() {
                ...
                if diagnostic_count <= params.max_diagnostics {
                    ...
                    diagnostics.push(tuna_diagnostics::serde::Diagnostic::new(error));
                }
            }
    ...
    diagnostics.extend(
        analyze_diagnostics
            .into_iter()
            .map(tuna_diagnostics::serde::Diagnostic::new)
            .collect::<Vec<_>>(),
    );
...
impl RegistryVisitor<JsLanguage> for ActionsVisitor<'_> {
    ...
    where
        R: tuna_analyze::Rule + 'static,
        R::Query: tuna_analyze::Queryable<Language = JsLanguage>,
        <R::Query as tuna_analyze::Queryable>::Output: Clone,
    {
...
fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    ...
    let file_source = parse
        .file_source(tuna_path)
        .map_err(|_| extension_error(params.tuna_path))?;
    ...
    let analyzer_options = compute_analyzer_options(&settings, PathBuf::from(tuna_path.as_path()));
    loop {
        ...
        match action {
            ...
            None => {
                let code = if should_format {
                    format_node(
                        settings.format_options::<JsLanguage>(tuna_path),
                        tree.syntax(),
                    )?
                    .print()?
                    .into_code()
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = tuna_js_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
fn format_on_type(
    ...
    let printed = tuna_js_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}
