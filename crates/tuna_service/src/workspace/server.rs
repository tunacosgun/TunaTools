...
use tuna_analyze::{AnalysisFilter, RuleFilter};
use tuna_diagnostics::{serde::Diagnostic as SerdeDiagnostic, Diagnostic, DiagnosticExt, Severity};
    ...
    fn get_parse(
        ...
        if ignored {
            return Err(WorkspaceError::file_ignored(format!(
                "{}",
                tuna_path.to_path_buf().display()
            )));
        }

        match self.syntax.entry(tuna_path) {
            Entry::Occupied(entry) => Ok(entry.get().clone()),
            Entry::Vacant(entry) => {
                let tuna_path = entry.key();
                let capabilities = self.get_capabilities(tuna_path);
                ...
                let parse = capabilities
                    .parser
                    .parse
                    .ok_or_else(self.build_capability_error(tuna_path))?;
                ...
                if size >= size_limit {
                    return Err(WorkspaceError::file_too_large(
                        tuna_path.to_path_buf().display().to_string(),
                        size,
                        size_limit,
                    ));
                }
...
impl Workspace for WorkspaceServer {
    ...
    fn is_path_ignored(&self, params: IsPathIgnoredParams) -> Result<bool, WorkspaceError> {
        ...
        let is_ignored_by_file_config = settings
            ...
            .files
            .ignored_files
            .matches_path(params.tuna_path.as_path());
