...
use tuna_console::fmt::Bytes;
use tuna_console::markup;
use tuna_diagnostics::{category, Category, Diagnostic, DiagnosticTags, Location, Severity, Visit};
use tuna_formatter::{FormatError, PrintError};
use tuna_fs::FileSystemDiagnostic;
use tuna_js_analyze::utils::rename::RenameError;
use tuna_js_analyze::RuleError;
...
impl Diagnostic for WorkspaceError {
    ...
    fn message(&self, fmt: &mut tuna_console::fmt::Formatter<'_>) -> std::io::Result<()> {
        match self {
...
mod test {
    ...
    use tuna_diagnostics::{print_diagnostic_to_string, DiagnosticExt, Error};
    use tuna_formatter::FormatError;
