...
pub fn main() -> io::Result<()> {
    ...
    let tokens = quote! {
        ...
        macro_rules! category {
            ...
            ( $name:literal ) => {
                compile_error!(concat!("Unregistered diagnostic category \"", $name, "\", please add it to \"crates/tuna_diagnostics_categories/src/categories.rs\""))
            };
            ( $( $parts:tt )* ) => {
        ...
        macro_rules! category_concat {
            ...
            ( @compile_error $( $parts:tt )* ) => {
                compile_error!(concat!("Unregistered diagnostic category \"", $( $parts, )* "\", please add it to \"crates/tuna_diagnostics_categories/src/categories.rs\""))
            };
            ( $( $parts:tt ),* ) => {
