    ...
    B: 'a,
{
    ...
    let mut analyzer = tuna_analyze::Analyzer::new(
        metadata(),
        tuna_analyze::InspectMatcher::new(registry, inspect_matcher),
    ...
    (
        analyzer.run(tuna_analyze::AnalyzerContext {
            root: root.clone(),
            range: filter.range,
            services,
            options,
...
#[cfg(test)]
mod tests {
    use tuna_analyze::{AnalyzerOptions, Never, RuleFilter};
    ...
    use tuna_json_syntax::TextRange;
    use std::slice;
