...
//! - `TSX` => `ts/` directory
//!
//! To create a new snapshot test for JavaScript, create a new file to `crates/tuna_js_formatter/tests/specs/js/`, e.g. `arrow_with_spaces.js`
//!
//! ```javascript
//! const foo     = ()    => {
//!     return bar
...
//!
//! ```bash
//! touch crates/tuna_js_formatter/tests/spec_tests.rs && cargo test -p tuna_js_formatter formatter
//! ```
//!
//! For better test driven development flow, start the formatter tests with [`cargo-watch`](https://crates.io/crates/cargo-watch):
//!
//! ```bash
//! cargo watch -i '*.new' -x 'test -p tuna_js_formatter formatter'
//! ```
//!
...
//!    to do it by using the formatter API [tuna_formatter::trivia::format_removed];
...
use tuna_formatter::format_element::tag::Label;
use tuna_formatter::prelude::*;
use tuna_formatter::{
    comments::Comments, write, CstFormatContext, Format, FormatLanguage, FormatToken,
    TransformSourceMap,
};
use tuna_formatter::{Buffer, FormatOwnedWithRule, FormatRefWithRule, Formatted, Printed};
use tuna_js_syntax::{
    AnyJsDeclaration, AnyJsStatement, JsLanguage, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken,
};
use tuna_rowan::TextRange;
...
pub(crate) trait AsFormat<Context> {
    type Format<'a>: tuna_formatter::Format<Context>
    where
        Self: 'a;
...
impl<T, C> AsFormat<C> for tuna_rowan::SyntaxResult<T>
where
    T: AsFormat<C>,
{
    type Format<'a> = tuna_rowan::SyntaxResult<T::Format<'a>> where Self: 'a;
...
pub(crate) trait IntoFormat<Context> {
    type Format: tuna_formatter::Format<Context>;

    fn into_format(self) -> Self::Format;
}

impl<T, Context> IntoFormat<Context> for tuna_rowan::SyntaxResult<T>
where
    T: IntoFormat<Context>,
{
    type Format = tuna_rowan::SyntaxResult<T::Format>;
    ...
    N: AstNode<Language = JsLanguage>,
{
    ...
    /// Formats the [leading comments](tuna_formatter::comments#leading-comments) of the node.
    ///
    ...
    /// Formats the [dangling comments](tuna_formatter::comments#dangling-comments) of the node.
    ///
    ...
    /// Formats the [trailing comments](tuna_formatter::comments#trailing-comments) of the node.
    ///
    /// You may want to override this method if you want to manually handle the formatting of comments
...
pub fn format_range(
    ...
    tuna_formatter::format_range(root, range, JsFormatLanguage::new(options))
}
...
pub fn format_node(
    ...
    tuna_formatter::format_node(root, JsFormatLanguage::new(options))
}
...
pub fn format_sub_tree(options: JsFormatOptions, root: &JsSyntaxNode) -> FormatResult<Printed> {
    tuna_formatter::format_sub_tree(root, JsFormatLanguage::new(options))
}
...
mod tests {
    ...
    use tuna_formatter::IndentStyle;
    use tuna_js_parser::{parse, parse_script, JsParserOptions};
    use tuna_js_syntax::JsFileSource;
    use tuna_rowan::{TextRange, TextSize};
