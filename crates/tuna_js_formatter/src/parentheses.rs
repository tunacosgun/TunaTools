...
use tuna_rowan::{declare_node_union, match_ast, AstNode, AstSeparatedList, SyntaxResult};
...
pub(crate) mod tests {
    ...
    use tuna_js_syntax::{JsFileSource, JsLanguage};
    ...
    >(
        ...
        let parse = tuna_js_parser::parse(input, source_type, JsParserOptions::default());
    ...
    ///
    /// ```
    /// # use tuna_js_formatter::assert_needs_parentheses;
    /// use tuna_js_syntax::JsStaticMemberExpression;
    ///
    ...
    macro_rules! assert_needs_parentheses {
        ($input:expr, $Node:ident) => {{
            $crate::assert_needs_parentheses!($input, $Node, tuna_js_syntax::JsFileSource::ts())
        }};
        ...
        ($input:expr, $Node:ident[$index:expr]) => {{
            $crate::assert_needs_parentheses!(
                $input,
                $Node[$index],
                tuna_js_syntax::JsFileSource::ts()
            )
        }};
    ...
    /// ```
    /// # use tuna_js_syntax::JsStaticMemberExpression;
    /// use tuna_js_formatter::assert_not_needs_parentheses;
    ///
    ...
    macro_rules! assert_not_needs_parentheses {
        ($input:expr, $Node:ident) => {{
            $crate::assert_not_needs_parentheses!($input, $Node, tuna_js_syntax::JsFileSource::ts())
        }};
