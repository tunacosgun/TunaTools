...
use tuna_formatter::{TransformSourceMap, TransformSourceMapBuilder};
use tuna_js_syntax::{
    AnyJsAssignment, AnyJsExpression, AnyTsType, JsLanguage, JsLogicalExpression, JsSyntaxKind,
    JsSyntaxNode,
};
use tuna_rowan::syntax::SyntaxTrivia;
use tuna_rowan::{
...
impl JsFormatSyntaxRewriter {
    ...
    /// trivia is slightly inconsistent with our [tuna_rowan::SyntaxToken::trailing_trivia] definition as it can now happen that the
                    ...
                    AnyJsExpression::JsLogicalExpression(right_logical) => {
                        match (
                                ...
                                if right_operator.kind() == operator.kind() =>
                            {
                                logical
                                    .with_left(
                                        tuna_js_factory::make::js_logical_expression(
...
mod tests {
    ...
    use tuna_formatter::{SourceMarker, TransformSourceMap};
    use tuna_js_parser::{parse, parse_module, JsParserOptions};
    use tuna_js_syntax::{
    ...
    use tuna_rowan::{AstNode, SyntaxRewriter, TextSize};
