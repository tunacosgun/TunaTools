use crate::prelude::*;
use tuna_formatter::{write, CstFormatContext, FormatRuleWithOptions, GroupId};

use crate::utils::array::write_array_node;

use crate::context::trailing_comma::FormatTrailingComma;
use tuna_js_syntax::JsArrayElementList;
use tuna_rowan::{AstNode, AstSeparatedList};
...
pub(crate) fn can_concisely_print_array_list(
    ...
    use tuna_js_syntax::AnyJsArrayElement::*;
    use tuna_js_syntax::AnyJsExpression::*;
    use tuna_js_syntax::JsUnaryOperator::*;
    ...
    list.elements().all(|item| {
        let syntax = match item.into_node() {
            Ok(AnyJsExpression(AnyJsLiteralExpression(
                tuna_js_syntax::AnyJsLiteralExpression::JsNumberLiteralExpression(literal),
            ))) => literal.into_syntax(),
