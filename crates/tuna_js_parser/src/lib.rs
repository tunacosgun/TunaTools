...
//! - Very easy tree traversal through [`SyntaxNode`](tuna_rowan::SyntaxNode).
...
//! The crate relies on a concept of untyped [tuna_js_syntax::JsSyntaxNode]s vs typed [tuna_rowan::AstNode]s.
//! Syntax nodes represent the syntax tree in an untyped way. They represent a location in an immutable
//! tree with two pointers. The syntax tree is composed of [tuna_js_syntax::JsSyntaxNode]s and [tuna_js_syntax::JsSyntaxToken]s in a nested
//! tree structure. Each node can have parents, siblings, children, descendants, etc.
//!
//! [tuna_rowan::AstNode]s represent a typed version of a syntax node. They have the same exact representation as syntax nodes
...
use tuna_js_factory::JsSyntaxFactory;
use tuna_js_syntax::{JsLanguage, JsSyntaxKind, LanguageVariant};
