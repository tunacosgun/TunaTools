...
use tuna_aria_metadata::AriaPropertiesEnum;
...
impl<'a> AriaRoles {
    ...
    pub fn get_implicit_role(
        &self,
        element: &str,
        // To generate `attributes`, you can use `tuna_js_analyze::aria_services::AriaServices::extract_defined_attributes`
...
mod test {
    ...
    fn test_get_implicit_role() {
        ...
        let implicit_role = aria_roles
            ...
            .unwrap();
        assert_eq!(implicit_role.type_name(), "tuna_aria::roles::ButtonRole");

        // <input type="search">
        let mut attributes = HashMap::new();
        attributes.insert("type".to_string(), vec!["search".to_string()]);
        let implicit_role = aria_roles.get_implicit_role("input", &attributes).unwrap();
        assert_eq!(implicit_role.type_name(), "tuna_aria::roles::SearchboxRole");
        ...
        assert_eq!(implicit_role.type_name(), "tuna_aria::roles::ListBoxRole");
    }
}
