...
pub(crate) fn generate_diagnostic(input: DeriveInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params tuna_diagnostics::Diagnostic for #ident #generics {
            #category
            #severity
            #description
            #message
...
fn generate_category(input: &DeriveInput) -> TokenStream {
    let category = match &input.category {
        Some(StaticOrDynamic::Static(value)) => quote! {
            tuna_diagnostics::category!(#value)
        },
    ...
    quote! {
        fn category(&self) -> Option<&'static tuna_diagnostics::Category> {
            Some(#category)
        }
    }
}
...
fn generate_severity(input: &DeriveInput) -> TokenStream {
    let severity = match &input.severity {
        Some(StaticOrDynamic::Static(value)) => quote! {
            tuna_diagnostics::Severity::#value
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
        },
    ...
    quote! {
        fn severity(&self) -> tuna_diagnostics::Severity {
            #severity
        }
    }
}
...
fn generate_description(input: &DeriveInput) -> TokenStream {
    let description = match &input.description {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            ...
            let write = tuna_diagnostics::termcolor::NoColor::new(&mut buffer);
            let mut write = tuna_diagnostics::console::fmt::Termcolor(write);
            let mut write = tuna_diagnostics::console::fmt::Formatter::new(&mut write);

            use tuna_diagnostics::console as tuna_console;
            write.write_markup(&tuna_diagnostics::console::markup!{ #markup })
...
fn generate_message(input: &DeriveInput) -> TokenStream {
    let message = match &input.message {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            use tuna_diagnostics::console as tuna_console;
            fmt.write_markup(tuna_diagnostics::console::markup!{ #markup })
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            tuna_diagnostics::console::fmt::Display::fmt(&self.#value, fmt)
        },
        None => return quote!(),
    };

    quote! {
        fn message(&self, fmt: &mut tuna_diagnostics::console::fmt::Formatter<'_>) -> ::std::io::Result<()> {
            #message
        }
    }
}
...
fn generate_advices(input: &DeriveInput) -> TokenStream {
    ...
    quote! {
        fn advices(&self, visitor: &mut dyn tuna_diagnostics::Visit) -> ::std::io::Result<()> {
            #( tuna_diagnostics::Advices::record(&self.#advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_verbose_advices(input: &DeriveInput) -> TokenStream {
    ...
    quote! {
        fn verbose_advices(&self, visitor: &mut dyn tuna_diagnostics::Visit) -> ::std::io::Result<()> {
            #( tuna_diagnostics::Advices::record(&self.#verbose_advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_location(input: &DeriveInput) -> TokenStream {
    ...
    quote! {
        fn location(&self) -> tuna_diagnostics::Location<'_> {
            tuna_diagnostics::Location::builder()
                #( .#method(&self.#field) )*
                .build()
        }
    }
...
fn generate_tags(input: &DeriveInput) -> TokenStream {
    let tags = match &input.tags {
        Some(StaticOrDynamic::Static(value)) => {
            ...
            quote! {
                #( tuna_diagnostics::DiagnosticTags::#values )|*
            }
        }
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
    ...
    quote! {
        fn tags(&self) -> tuna_diagnostics::DiagnosticTags {
            #tags
        }
    }
}
...
fn generate_source(input: &DeriveInput) -> TokenStream {
    match &input.source {
        Some(value) => quote! {
            fn source(&self) -> Option<&dyn tuna_diagnostics::Diagnostic> {
                self.#value.as_deref()
            }
        },
        None => quote!(),
